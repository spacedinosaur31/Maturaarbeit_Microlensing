...
for i in range(LCamount):   
    ...
    if ML_yesorno == 1: #falls Microlensing-Event
        ...
        for x in range(magpointamount):  # Helligkeit A abhängig von u, u abhängig t
            t_0 = 0
            u = np.sqrt(umin**2 + ((t[x]-t_0)/tE)**2)
            A = mean_luminosity*((u**2 + 2) / (u*np.sqrt(u**2 + 4))) 
            M = -2.5*np.log10(A) + random.gauss(-mean_std, mean_std) # bei beiden Gauss-Rauschen machen
            mag[x] = M
        ...
        # FILTER HERE:
        if lc_neumann_value <= 10**((lc_skew_value - 1.3)/-0.4) - 1.6: # no log() because then skew can't be negative but should
            found += 1
        ...
        # FILTER HERE:
        if lc_neumann_value <= (10**((lc_skew_value - 1.3)/-0.4) - 1.6):
            trap += 1     
        # plt.figure() # make coordinate system
...
y = [0.01*y for y in range(110, 300)]
x = []

for i in y:
    x.append(10**((i - 1.3)/-0.4) - 1.6)
