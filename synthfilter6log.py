...
for a in [0.01*x for x in range(-100, 5]: # in range of sensible parameters
   for b in [0.01*x for x in range(100, 200)]:
      for c in [0.01*x for x in range(50, 200)]:
   ...
   # SYNTHETIC LC GENERATOR + FILTERING -> Produktion künstlicher LC -> random ML-Events oder nicht 
         for i in range(LCamount):   
             ...
             if ML_yesorno == 1: #falls Microlensing-Event
                 ...
                 if lc_skew_value <= (10**((lc_neumann_value - c)/a) - b):
                     trap += 1
         param_combinations.append([a, b, c])
...
print(max(foundlostrelations), skew
